...
namespace XazeAPI.Patches
{
    ...
    public class PreventHitmarker
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            // Remove the final two instructions: ldc.i4.1 + ret
            newInstructions.RemoveAt(newInstructions.Count - 1); // ret
            newInstructions.RemoveAt(newInstructions.Count - 1); // ldc.i4.1

            // Inject:
            // return CustomPreventHitmarker(adh, victim);
            newInstructions.AddRange(new[]
            {
                new CodeInstruction(OpCodes.Ldarg_0), // adh
                new CodeInstruction(OpCodes.Ldarg_1), // victim
